...
public class RoomZero : MonoBehaviour
{
    ...
    public SteamVR_Action_Boolean DS_Trigger_Action;
    ...
    void Update()
    {
        ...
        if (!stop)
        {
            SaveInfo.roomStatus.SetCondition(SaveInfo.StroopRoomCondition.ZERO);
            ...
            if (Physics.Raycast(raycast, out hit) && DS_Trigger_Action.stateDown)
            {

                if (hit.collider.CompareTag("TARGET"))
                {
                    //File.AppendAllText(path, hit.collider.gameObject.name + ",Right,");
                    Right(hit.collider.gameObject.name);
                    
                }
                else
                {
                    //File.AppendAllText(path, hit.collider.gameObject.name + ",Wrong,");
